...
if __name__ == "__main__":
    ...
    print(mapa)
    assert mapa.keeper == (5, 3)
    assert mapa.get_tile((4, 2)) == Tiles.GOAL
    assert mapa.get_tile((5, 2)) == Tiles.FLOOR
    #assert mapa.get_tile((2, 7)) == Tiles.BOX
    assert mapa.get_tile(mapa.keeper) == Tiles.MAN
    # Fake move:
    keeper_pos = mapa.keeper
    mapa.clear_tile(keeper_pos)
    mapa.set_tile(keeper_pos, Tiles.FLOOR)
    mapa.clear_tile((keeper_pos[0], keeper_pos[1] + 1))
    mapa.set_tile((keeper_pos[0], keeper_pos[1] + 1), Tiles.MAN)
    assert mapa.keeper == (5, 4)
    assert mapa.get_tile((2, 3)) == Tiles.BOX
    assert mapa.on_goal == 0
    assert mapa.boxes == [(2, 3), (3, 3)]
    """
